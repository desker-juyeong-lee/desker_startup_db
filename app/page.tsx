...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<"table"|"dashboard"|"cache">("table");
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (e) => {
      ...
      const initialStates: RowState[] = dataRows.map((r, i) => {
        ...
        if (!isValidCompanyName(name)) { skipped++; return { name, status: "done" as RowStatus }; }
        const wasError = (r[11] || "").includes("(오류)");
        if (wasError) return { name, status: "wait" as RowStatus };
      ...
      addLog(`파일 로드: ${dataRows.length}개 행 | 유효 기업: ${dataRows.length - skipped}개 | 캐시: ${preHit}개 | 칸밀림 스킵: ${skipped}개`, "ok");
    };
  ...
  async function processOne(i: number, total: number, states: RowState[]): Promise<void> {
    ...
    if (states[i]?.status === "cached") { setDoneCount(d => d + 1); return; }
  ...
  function handleCacheDownload() {
    ...
    const dataRows = cacheEntries.map(([name, e]) => [name, e.address, e.mate, String(e.hire_count), e.date]);
    const csv = "\uFEFF" + toCSVText([hdr, ...dataRows]);
    const blob = new Blob([csv], { type:"text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
  ...
  function handleDeleteCacheEntry(name: string) {
  ...
  function handleClearCache() {
    ...
    cacheRef.current = {}; saveCache({}); refreshCacheEntries();
  ...
  const filteredCache = cacheSearch.trim()
    ? cacheEntries.filter(([name, e]) => name.includes(cacheSearch) || e.address.includes(cacheSearch) || e.mate.includes(cacheSearch))
    : cacheEntries;
      ...
      <div style={S.card}>
        ...
        {total > 0 && (
          <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:8,marginBottom:"1rem"}}>
            {([["전체행",total,"#444"],["유효기업",validTotal,"#333"],["완료",doneCount,"#0a7c55"],["캐시",cachedCount,"#534AB7"],["처리중",procCount,"#8a5200"],["오류",errCount,"#a02020"]] as [string,number,string][]).map(([label,val,color])=>(
        ...
        {skippedCount > 0 && (
          <div style={{background:"#fffbe6",border:"1px solid #ffe58f",borderRadius:8,padding:"8px 14px",fontSize:12,color:"#8a5200",marginBottom:12}}>
            ⚠ 칸 밀림으로 인한 비정상 행 {skippedCount}개 감지 — 자동 스킵
          </div>
        )}
        ...
        <div style={{display:"flex",gap:0,marginBottom:16,borderBottom:"1px solid #eee"}}>
          {([
            ["table","📋 기업 목록", total > 0],
            ["dashboard","📊 MATE 대시보드", total > 0],
            ["cache",`💾 캐시 DB (${cacheSize})`, true],
          ] as [string, string, boolean][]).map(([tab, label, show])=> show && (
            <button key={tab} onClick={()=>setActiveTab(tab as "table"|"dashboard"|"cache")} style={{
                  ...
                  return (
                    <div key={mate} style={{background:"white",border:"1px solid #eee",borderRadius:10,padding:"1rem",borderTop:`3px solid ${color}`}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {companies.map(c=><span key={c} style={{fontSize:11,background:"#f5f5f5",color:"#555",borderRadius:4,padding:"2px 7px"}}>{c}</span>)}
                      </div>
                    </div>
                  );
                })}
        ...
        {activeTab==="cache" && (
          <div>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:14,flexWrap:"wrap"}}>
              ...
              />
              <button style={S.btnSecondary} onClick={handleCacheDownload} disabled={cacheEntries.length===0}>
                ⬇ 캐시 CSV 다운로드
              </button>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
              {([
                ["저장된 기업", cacheSize+"개", "#534AB7"],
                ["오늘 업데이트", cacheEntries.filter(([,e])=>e.date===today).length+"개", "#0a7c55"],
                ["검색 결과", filteredCache.length+"개", "#444"],
            ...
            {cacheEntries.length === 0 ? (
              <div style={{textAlign:"center",padding:"3rem",color:"#aaa",fontSize:14}}>
                ...
                아직 캐시된 데이터가 없습니다.<br/>자동 채우기 실행 후 조회 결과가 여기에 저장됩니다.
              </div>
            ) : (
              <div style={S.tableWrap}>
                <table style={S.table}>
                  <thead>
                    <tr>
                      {["기업명","본사 지역","MATE 매칭","최근 1년 채용건수","업데이트 일자",""].map(h=><th key={h} style={S.th}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={name} style={{background:isToday?"#f0faf5":"white"}}>
                          ...
                          <td style={{...S.td,maxWidth:140}}>
                            {entry.mate
                              ?<span style={{background:mateColor+"18",color:mateColor,borderRadius:99,padding:"1px 8px",fontSize:11,fontWeight:500}}>{entry.mate}</span>
                              :"-"}
                          </td>
                          <td style={{...S.td,textAlign:"center"}}>
                            <span style={{background:"#f0faf5",color:"#0a7c55",borderRadius:99,padding:"1px 10px",fontSize:12,fontWeight:600}}>{entry.hire_count}건</span>
                          </td>
                          <td style={{...S.td,whiteSpace:"nowrap"}}>
                            <span style={{color:isToday?"#0a7c55":"#888",fontSize:12}}>{isToday?"🟢 오늘":entry.date}</span>
                          </td>
                          <td style={{...S.td,textAlign:"center"}}>
                            <button onClick={()=>handleDeleteCacheEntry(name)} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:14,padding:"0 4px"}} title="삭제">✕</button>
                          </td>
                        </tr>
                      );
                    })}
