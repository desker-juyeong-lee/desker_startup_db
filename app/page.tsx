...
function isValidCompanyName(name:string):boolean {
  ...
  if(n.includes(',')) return false; // ← 쉼표 포함 무조건 삭제
  ...
  if(stageKw.some(k=>n.toLowerCase()===k)) return false;
...
export default function Home() {
  ...
  function handleFile(file:File){
    ...
    reader.onload=(e)=>{
      ...
      const initStates:RowState[]=dataRows.map((r,i)=>{
        ...
        if(!isValidCompanyName(name)){skipped++;return{name,status:"skip"};}
        const wasError=(r[11]||"").includes("(오류)");
        if(wasError) return{name,status:"wait"};
      ...
      setLogs([]); setDoneCount(0); setErrCount(0); setCachedCount(preHit); setSkippedCount(skipped);
      addLog(`파일 로드: ${dataRows.length}행 | 유효: ${dataRows.length-skipped}개 | 캐시: ${preHit}개 | 스킵: ${skipped}개`,"ok");
    };
  ...
  async function runAll(){
    setRunning(true); stopRef.current=false; pauseRef.current=false; setDoneCount(0);
  ...
  function handleDownload(){
    const exportRows=rowsRef.current.filter(r=>!(r[11]||"").includes("(주소없음-삭제)")).map(r=>{
  ...
  return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        {skippedCount>0&&(<div style={{background:"#fffbe6",border:"1px solid #ffe58f",borderRadius:8,padding:"8px 14px",fontSize:12,color:"#8a5200",marginBottom:12}}>⚠ 쉼표 포함/칸밀림 비정상 행 {skippedCount}개 자동 스킵</div>)}
        ...
        {total>0&&(
          <div style={S.btnRow}>
            {!running?(<button style={S.btnPrimary} onClick={runAll}>▶ 자동 채우기 실행</button>):(
            ...
            <table style={S.table}>
              ...
              <tbody>
                {rows.map((row,i)=>{
                  ...
                  const isDeleted=(row[11]||"").includes("주소없음-삭제");
                  ...
                  return(
                    <tr key={i} style={{background:bg,opacity:isSkip&&!isDeleted?0.45:1}}>
                      ...
                      <td style={{...S.td,maxWidth:220,color:"#a02020",fontSize:11}}>{isErr?(st?.error||"-"):isDeleted?"주소 미확인":"-"}</td>
                    </tr>
                  );
                })}
              </tbody>
